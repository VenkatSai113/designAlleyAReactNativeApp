...
import EditEstimateTables from './editEstimateTable'
...
const EditEstimate=({route})=>{
    ...
    const [marginData,setMarginData]=useState(marginItem)
    ...
    useEffect(()=>{
        ...
        let i=0
            data.map(item=>
               ...
               if(response.ok===true){
                const handleEstimationDetails=async()=>{
                    const jwtToken=await AsyncStorage.getItem('jwtToken')
                    const apiUrl='http://192.168.1.36:9000/editEstimationDetails'
                    const spaces={spaceIdArray}
                    const options={
                        method:'POST',
                        headers:{
                            'Content-Type':'Application/json',
                    ...
                    const response=await fetch(apiUrl,options)
                    const data=await response.json()
                    console.log(data,'ESTIMATEEEE')
                    data.forEach(item=>
                        marginItem.push(item.margin),
                        setMarginData(marginItem)
                     )
                    
                }
                handleEstimationDetails()
               }
    ...
    const handleEditEstimate=()=>{
        navigation.navigate('EditEstimate', { screen: 'EditEstimate' });
    ...
    return(
      ...
      <EditEstimateTables spaceProducts={eachItem} index={index} totalPrices={totalPrices} key={eachItem.index} marginItem={marginData} />
        )}
         <DataTable.Row> 
